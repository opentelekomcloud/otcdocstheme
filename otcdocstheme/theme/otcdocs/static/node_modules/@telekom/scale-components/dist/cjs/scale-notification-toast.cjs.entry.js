...
const NotificationToast = class {
  ...
  handleIcons() {
    if (this.variant) {
      switch (this.variant) {
        case 'success':
          return (index.h("scale-icon-alert-success", { class: "notification-toast__icon", size: 20, color: "#ffffff", selected: true, "accessibility-title": "success" }));
        case 'informational':
          return (index.h("scale-icon-alert-information", { class: "notification-toast__icon", size: 20, selected: true, color: "#ffffff", "accessibility-title": "information" }));
        case 'error':
          return (index.h("scale-icon-alert-error", { class: "notification-toast__icon", size: 20, selected: true, color: "#ffffff", "accessibility-title": "error" }));
        case 'warning':
          return (index.h("scale-icon-alert-error", { class: "notification-toast__icon", color: "#ffff", size: 20, selected: true, "accessibility-title": "information" }));
      }
    }
    return;
  }
  ...
  render() {
    if (this.opened) {
      return (index.h(index.Host, null, this.styles && index.h("style", null, this.styles), index.h("style", null, this.transitions(this.toastHeightWithOffset)), index.h("style", null, this.animationStyle(this.toastHeightWithOffset)), index.h("div", { class: this.getCssClassMap(), part: this.getBasePartMap() }, index.h("div", { class: "notification-toast__icon-container" }, this.handleIcons()), index.h("div", { class: "notification-toast__text-container" }, index.h("slot", { name: "header" }), index.h("slot", { name: "body" }), index.h("scale-link", null, index.h("slot", { name: "link" }))), index.h("scale-icon-action-circle-close", { tabindex: "0", class: "notification-message__icon-close", size: 20, onClick: () => {
          this.close();
        }, onKeyDown: (e) => {
          if (e.key === 'Enter') {
            this.close();
